        ...
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          {done ? (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-ibe-cream rounded-2xl p-8 md:p-12 border border-ibe-bark/5 shadow-lg shadow-ibe-bark/5 text-center">
              <div className="w-16 h-16 rounded-full bg-ibe-ever mx-auto mb-5 flex items-center justify-center"><Check className="text-ibe-spring" size={30} /></div>
              <h2 className="font-lora text-ibe-ever text-3xl font-bold mb-2">Thanks, {d.name.split(" ")[0]}! Your quote is in.</h2>
            ...
            <div className="bg-ibe-cream rounded-2xl p-6 md:p-9 border border-ibe-bark/5 shadow-lg shadow-ibe-bark/5">
              ...
              <div className="flex items-center gap-2 mb-8">
                {STEPS.map((s, i) => (
                  <div key={s} className="flex-1">
                    <div className={`h-1.5 rounded-full transition ${step > i ? "bg-ibe-ever" : step === i + 1 ? "bg-ibe-spring" : "bg-ibe-bark/10"}`} />
                    <div className={`mt-2 font-mulish text-xs font-bold uppercase tracking-wider ${step > i ? "text-ibe-ever" : "text-ibe-bark/40"}`}>{i + 1}. {s}</div>
                  </div>
                ))}
              </div>
              ...
              {step === 1 && (
                <motion.div key="s1" initial="hidden" animate="show" variants={fade} className="space-y-6">
                  <h2 className="font-lora text-ibe-ever text-2xl font-bold">Tell us about your property</h2>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={d.gated} onChange={(e) => set("gated", e.target.checked)} className="w-5 h-5 accent-ibe-ever" /><span className="font-mulish text-ibe-bark">Fenced / locked gate access</span></label>
              ...
              {step === 2 && (
                <motion.div key="s2" initial="hidden" animate="show" variants={fade} className="space-y-6">
                  <h2 className="font-lora text-ibe-ever text-2xl font-bold">What services do you need?</h2>
                  <p className="text-ibe-bark/60 font-mulish -mt-3">Select all that apply.</p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {SVCS.map((s) => {
                      const on = d.services.includes(s);
                      return (
                        <button type="button" key={s} onClick={() => toggleSvc(s)} className={`flex items-center gap-3 p-4 rounded-xl border text-left transition ${on ? "border-ibe-ever bg-ibe-ever/5" : "border-ibe-bark/15 bg-white hover:border-ibe-ever/40"}`}>
                          <span className={`w-6 h-6 rounded-md grid place-items-center shrink-0 ${on ? "bg-ibe-ever" : "bg-ibe-bark/10"}`}><Check className={on ? "text-ibe-spring" : "text-transparent"} size={14} /></span>
              ...
              {step === 3 && (
                <motion.div key="s3" initial="hidden" animate="show" variants={fade} className="space-y-6">
                  <h2 className="font-lora text-ibe-ever text-2xl font-bold">Schedule & contact</h2>
              ...
              <div className="flex items-center justify-between mt-9 pt-6 border-t border-ibe-bark/10">
                {step > 1 ? <button type="button" onClick={() => setStep((s) => s - 1)} className="inline-flex items-center gap-2 px-5 py-3 rounded-full text-ibe-ever font-mulish font-bold hover:bg-ibe-ever/5 transition"><ArrowLeft size={18} /> Back</button> : <span />}
                <button type="button" onClick={step === 3 ? submit : next} disabled={submitting || (step === 1 ? !s1ok : step === 2 ? !s2ok : !s3ok)} className={`inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-mulish font-bold transition ${submitting || (step === 1 ? !s1ok : step === 2 ? !s2ok : !s3ok) ? "bg-ibe-bark/15 text-ibe-bark/40 cursor-not-allowed" : "bg-ibe-spring text-ibe-bark hover:bg-ibe-ever hover:text-ibe-cream"}`}>
...
function Field({ label, children }) {
  return (<label className="block"><span className="block text-ibe-ever/60 text-xs font-mulish font-bold uppercase tracking-[0.15em] mb-2">{label}</span>{children}</label>);
}
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {opts.map((o) => {
        ...
        return (
          <button type="button" key={o.label} onClick={() => onPick(o.label)} className={`text-left p-3 rounded-xl border transition ${active ? "border-ibe-ever bg-ibe-ever/5" : "border-ibe-bark/15 bg-white hover:border-ibe-ever/40"}`}>
            <div className="flex items-center justify-between mb-0.5"><span className="font-mulish font-semibold text-ibe-bark text-sm">{o.label}</span><span className={`w-5 h-5 rounded-full border-2 grid place-items-center ${active ? "border-ibe-ever bg-ibe-ever" : "border-ibe-bark/25"}`}>{active && <Check size={11} className="text-ibe-spring" />}</span></div>
