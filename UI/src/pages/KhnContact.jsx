      ...
      <section className="bg-ibe-bone py-20">
        <div className="max-w-[1320px] mx-auto px-6 lg:px-8 grid lg:grid-cols-5 gap-10">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }} className="lg:col-span-3">
            <h2 className="font-lora text-knova-midnight text-3xl font-bold mb-2">Send Us a Message</h2>
            ...
            <div className="bg-ibe-cream rounded-2xl p-7 md:p-9 border border-ibe-bark/5 shadow-lg shadow-ibe-bark/5">
              {status === "success" ? (
                <div className="text-center py-12">
                  <div className="w-16 h-16 rounded-full bg-knova-midnight mx-auto mb-5 flex items-center justify-center"><CheckCircle2 className="text-knova-cyan" size={30} /></div>
                  <h3 className="font-lora text-knova-midnight text-2xl font-bold mb-2">Message sent!</h3>
                ...
                <form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2">
                    <button type="submit" disabled={status === "submitting"} className="inline-flex w-full items-center justify-center gap-2 px-8 py-4 rounded-full bg-knova-cyan text-ibe-bark font-mulish font-bold hover:bg-knova-midnight hover:text-ibe-cream transition disabled:opacity-60">
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6, delay: 0.1 }} className="lg:col-span-2">
            <h2 className="font-lora text-knova-midnight text-3xl font-bold mb-2">Get In Touch</h2>
            ...
            <div className="space-y-4">
              {INFO.map((i) => (
                <div key={i.label} className="bg-ibe-cream rounded-2xl p-6 border border-ibe-bark/5 flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-knova-midnight/10 grid place-items-center shrink-0"><i.icon className="text-knova-midnight" size={22} /></div>
                  <div>
                    <div className="text-ibe-bark/50 font-mulish text-xs uppercase tracking-wider">{i.label}</div>
                    {i.href ? <a href={i.href} className="text-knova-midnight font-mulish font-semibold hover:text-knova-cyan break-all">{i.value}</a> : <div className="text-ibe-bark font-mulish font-semibold">{i.value}</div>}
                  </div>
                </div>
              ))}
            </div>
...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="block text-knova-midnight/60 text-xs font-mulish font-bold uppercase tracking-[0.15em] mb-2">{label}</span>
      {children}
    </label>
  );
}
