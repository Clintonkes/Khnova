...
type Booking = {
  ...
  service: string | null; lawn_size: string | null; preferred_date: string | null;
...
function AdminPage() {
  ...
  useEffect(() => {
    const token = localStorage.getItem("olson_admin_token");
    if (!token) {
      setChecking(false);
      return;
    }
    apiGet<AdminInfo>("/api/admin/me")
      ...
      .catch(() => {
        localStorage.removeItem("olson_admin_token");
      })
...
function AuthGate({ onLogin }: { onLogin: (info: AdminInfo) => void }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("olson_admin_token", access_token);
    ...
    <div className="grid min-h-screen place-items-center bg-gradient-to-br from-secondary/40 via-background to-background px-6">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-8 flex items-center justify-center gap-2.5">
          ...
          <span className="font-display text-xl font-extrabold">Olson <span className="text-primary">LLC</span></span>
        </Link>
      ...
      <header className="border-b border-primary/10 bg-background">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={signOut} className="inline-flex items-center gap-1.5 rounded-lg border border-primary/15 px-3 py-2 text-xs font-bold hover:bg-secondary">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {new Date(b.created_at).toLocaleString()} · <span className="font-semibold text-primary">{b.service ?? "General Service"}</span>
          </p>
        </div>
